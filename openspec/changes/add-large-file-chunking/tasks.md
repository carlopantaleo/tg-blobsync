...
- [x] 3.3 Implement a streaming chunk reader (`chunkReader`) that opens the next chunk lazily and concatenates via `io.MultiReader`-style chaining without buffering whole chunks
- [x] 3.4 Tests: concatenated reader yields exact original bytes for a multi-chunk fixture; missing chunk returns error; reader closes underlying chunk readers on close

## 4. Telegram adapter - upload (TDD)

- [x] 4.1 Refactor `UploadFile` to branch on `file.Size > chunkThreshold`
- [x] 4.2 Implement chunked upload: split via `chunkPlan`, upload each chunk with `Flags=CHUNK` and `Idx`, collect returned message IDs
...
- [x] 6.1 Extend `DeleteFile` to accept `chunkIDs []int` and delete all of them
- [x] 6.2 Update `parseMessageToFile` to parse `Idx` and `CHUNK` flag
- [x] 6.3 Update `ListFiles` legacy fallback to group `CHUNK` messages into one `RemoteFile` with `ChunkIDs` sorted by `Idx`; grouping key is `Path`+`Checksum` when `Checksum` is non-empty, else `Path` alone (covers `--skip-md5`); skip incomplete sets
- [x] 6.4 Tests: fallback groups chunks into one `RemoteFile` both with and without `Checksum`; orphan/incomplete chunk set is skipped

## 7. Topic index integration (TDD)

- [x] 7.1 `UploadIndex` serializes `chunkIDs` for chunked entries
- [x] 7.2 `GetIndex` recovery builds `RemoteFile` with `ChunkIDs` and total size for chunked entries
- [x] 7.3 Tests: index round-trip with a mix of non-chunked and chunked entries preserves `chunkIDs` order and sizes
...
- [x] 9.2 Tests: progress task with chunk info renders the chunk indicator; non-chunked tasks render unchanged
